...
class RoboteqCommand(object):
    u"""Class to use as generic class for a roboteq command"""
    __slots__ = (u'Identity', u'HexID', u'Name', u'Function', u'Aliases')

    def __init__(self, _Identity, _HexID, _Name=u'', _Function=u'', _Aliases=None):
        # TODO consider making Requied underlying values immutable after they are initialized, or at least private
        u"""Constructor: A Roboteq Command
        Required Values:
        ...
        if isinstance(_Identity, unicode):
            self.Identity = _Identity
        else:
            raise TypeError(u"Alias must be a String")
            # TODO check that Alias is all CppHeaderParser
            # TODO add support for multiple aliases
        if isinstance(_HexID, int) and _HexID < 255:
            self.HexID = _HexID
        else:
            raise TypeError(u"HexID must be a positive number 255 or below")
...
class RoboteqCommandLibrary(dict):
    u"""This structure is going to have to hold a set of RoboteQ commands that can be accessed using their identity stringself.
...
class RoboteqCommander(object):
    u"""This is the core Roboteq Command class, with a generic RoboteqCommander Being an instantiation set up to use basic StringIO
    ...
    def setCommand(self, token, *args):
        return self.Command(u'!', token, *args)
